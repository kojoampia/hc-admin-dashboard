...
type RestOf<T extends IAuditLog | NewAuditLog> = Omit<T, 'createdDate' | 'modifiedDate'> & {
...
export class AuditLogService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/audit-logs', 'hcadminservice');
  ...
  protected convertDateFromClient<T extends IAuditLog | NewAuditLog | PartialUpdateAuditLog>(auditLog: T): RestOf<T> {
    return {
      ...auditLog,
      createdDate: auditLog.createdDate?.toJSON() ?? null,
      modifiedDate: auditLog.modifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAuditLog: RestAuditLog): IAuditLog {
    return {
      ...restAuditLog,
      createdDate: restAuditLog.createdDate ? dayjs(restAuditLog.createdDate) : undefined,
      modifiedDate: restAuditLog.modifiedDate ? dayjs(restAuditLog.modifiedDate) : undefined,
    };
  }
