...
type RestOf<T extends IPerson | NewPerson> = Omit<T, 'birthDate' | 'createdDate' | 'modifiedDate'> & {
...
export class PersonService {
  ...
  protected convertDateFromClient<T extends IPerson | NewPerson | PartialUpdatePerson>(person: T): RestOf<T> {
    return {
      ...
      createdDate: person.createdDate?.toJSON() ?? null,
      modifiedDate: person.modifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPerson: RestPerson): IPerson {
    return {
      ...
      createdDate: restPerson.createdDate ? dayjs(restPerson.createdDate) : undefined,
      modifiedDate: restPerson.modifiedDate ? dayjs(restPerson.modifiedDate) : undefined,
    };
  }
