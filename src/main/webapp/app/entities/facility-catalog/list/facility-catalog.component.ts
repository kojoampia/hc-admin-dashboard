...
export class FacilityCatalogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (this.facilityCatalogs().length === 0) {
            this.load();
          } else {
            this.facilityCatalogs.set(this.refineData(this.facilityCatalogs()));
          }
        }),
      )
      .subscribe();
  ...
  delete(facilityCatalog: IFacilityCatalog): void {
    const modalRef = this.modalService.open(FacilityCatalogDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.facilityCatalog = facilityCatalog;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.facilityCatalogs.set(this.refineData(dataFromBody));
  }

  protected refineData(data: IFacilityCatalog[]): IFacilityCatalog[] {
