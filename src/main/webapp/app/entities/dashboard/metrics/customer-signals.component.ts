        ...
        <div class="space-y-2">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p class="text-xs text-hpd-subtle">{{ signal.trend }}</p>
            </div>
            <span class="rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-hpd-on-navy-muted">{{ signal.score }}</span>
          </div>
          <div class="h-2 overflow-hidden rounded-full bg-hpd-primary">
            <div class="h-full rounded-full bg-hpd-gold transition-all" [style.width.%]="signal.progress"></div>
          </div>
        </div>
      }
    </div>
