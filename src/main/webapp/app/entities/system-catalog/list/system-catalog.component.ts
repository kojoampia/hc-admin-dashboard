...
import { SystemCatalogDeleteDialogComponent } from '../delete/system-catalog-delete-dialog.component';
import { EntityArrayResponseType, SystemCatalogService } from '../service/system-catalog.service';
...
export class SystemCatalogComponent implements OnInit {
  ...
  systemCatalogs = signal<ISystemCatalog[]>([]);
  isLoading = false;
  ...
  protected readonly systemCatalogService = inject(SystemCatalogService);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.systemCatalogs.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: ISystemCatalog[] | null): ISystemCatalog[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const systemCatalogsNew = this.systemCatalogs();
      if (data) {
        for (const d of data) {
          if (systemCatalogsNew.some(op => op.id === d.id)) {
            systemCatalogsNew.push(d);
          }
        }
      }
      return systemCatalogsNew;
    }
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.systemCatalogService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
