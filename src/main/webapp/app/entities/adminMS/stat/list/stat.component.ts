...
export class StatComponent implements OnInit {
  ...
  delete(stat: IStat): void {
    const modalRef = this.modalService.open(StatDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.stat = stat;
    // unsubscribe not needed because closed completes on modal close
    modalRef.closed
      .pipe(
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.stats = this.refineData(dataFromBody);
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IStat[] | null): IStat[] {
    return data ?? [];
  }
