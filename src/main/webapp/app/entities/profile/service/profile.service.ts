...
type RestOf<T extends IProfile | NewProfile> = Omit<T, 'createdDate' | 'modifiedDate'> & {
...
export class ProfileService {
  ...
  protected convertDateFromClient<T extends IProfile | NewProfile | PartialUpdateProfile>(profile: T): RestOf<T> {
    return {
      ...
      modifiedDate: profile.modifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProfile: RestProfile): IProfile {
    return {
      ...
      modifiedDate: restProfile.modifiedDate ? dayjs(restProfile.modifiedDate) : undefined,
    };
  }
