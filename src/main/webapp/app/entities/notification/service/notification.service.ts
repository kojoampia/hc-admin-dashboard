...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'createdDate' | 'modifiedDate'> & {
...
export class NotificationService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/notifications', 'hcadminservice');
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...
      modifiedDate: notification.modifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...
      modifiedDate: restNotification.modifiedDate ? dayjs(restNotification.modifiedDate) : undefined,
    };
  }
