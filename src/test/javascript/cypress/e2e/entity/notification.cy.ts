...
describe('Notification e2e test', () => {
  ...
  const notificationSample = {
    content: 'underneath astride nor',
    recipientId: 'that abaft',
    senderId: 'populist',
    messageType: 'once vein',
  };
  ...
  describe('new Notification page', () => {
    ...
    it('should create an instance of Notification', () => {
      cy.get(`[data-cy="content"]`).type('butter modulo');
      cy.get(`[data-cy="content"]`).should('have.value', 'butter modulo');

      cy.get(`[data-cy="recipientId"]`).type('however');
      cy.get(`[data-cy="recipientId"]`).should('have.value', 'however');

      cy.get(`[data-cy="senderId"]`).type('but yet chunder');
      cy.get(`[data-cy="senderId"]`).should('have.value', 'but yet chunder');

      cy.get(`[data-cy="messageType"]`).type('unwelcome ouch');
      cy.get(`[data-cy="messageType"]`).should('have.value', 'unwelcome ouch');

      cy.get(`[data-cy="createdDate"]`).type('2026-05-12T09:44');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2026-05-12T09:44');

      cy.get(`[data-cy="modifiedDate"]`).type('2026-05-12T16:45');
      cy.get(`[data-cy="modifiedDate"]`).blur();
      cy.get(`[data-cy="modifiedDate"]`).should('have.value', '2026-05-12T16:45');
